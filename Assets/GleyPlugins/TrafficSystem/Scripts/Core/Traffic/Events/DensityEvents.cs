...
namespace GleyTrafficSystem
{
    public static class DensityEvents
    {
        ...
        public delegate void VehicleAdded(int vehicleIndex, Vector3 targetWaypointPosition, float maxSpeed, float powerStep, float brakeStep);
        public static VehicleAdded onVehicleAdded;
        public static void TriggerVehicleAddedEvent(int vehicleIndex, Vector3 targetWaypointPosition, float maxSpeed, float powerStep, float brakeStep)
        {
            if (onVehicleAdded != null)
            {
                onVehicleAdded(vehicleIndex, targetWaypointPosition, maxSpeed, powerStep, brakeStep);
            }
        }
    }
}