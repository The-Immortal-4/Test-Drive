...
namespace GleyTrafficSystem
{
    ...
    public struct DriveJob : IJobParallelFor
    {
        ...
        public void Execute(int index)
        {
            ...
            localMaxSpeed = SetMaxSpeed(index, turnAngle[index], maxSteer[index], maxSpeed[index]);

            //compute acceleration based on the current vehicle actions
            ComputeForces(index);

            //compute target speed for next frame
            nextFrameSpeed = GetSpeed(index, targetSpeed, localMaxSpeed, gear[index]);
        ...
        private float SetMaxSpeed(int index, float turnAngle, float maxSteer, float maxSpeed)
        {
            ...
            if (localMaxSpeed / targetSpeed < 2)
            {
                if (turnAngle > 3)
                {
            ...
            if (targetSpeed > localMaxSpeed)
            {
                if (targetSpeed - localMaxSpeed > 1)
                {
                ...
                if (specialDriveAction[index] == SpecialDriveActionTypes.Follow || specialDriveAction[index] == SpecialDriveActionTypes.Overtake)
                {
                    SetFollowSpeed(index, localMaxSpeed);
                }
                else
                {
                    ApplyBrakes(index, 1);
        ...
        /// <param name="localMaxSpeed"></param>
        private void SetFollowSpeed(int index, float localMaxSpeed)
        {
            //compute per frame velocity
            float velocityPerFrame = (targetSpeed - localMaxSpeed) * fixedDeltaTime;
            ...
            int brakeNrOfFrames = (int)((targetSpeed - localMaxSpeed) / brakeStep[index]);
