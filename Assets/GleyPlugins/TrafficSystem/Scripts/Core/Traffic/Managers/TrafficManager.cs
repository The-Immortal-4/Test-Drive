...
namespace GleyTrafficSystem
{
    ...
    public class TrafficManager : MonoBehaviour
    {
        ...
        private NativeArray<int> vehicleEndWheelIndex; //nr of wheels that car with index i has (nrOfCars)
        ...
        public void Initialize(Transform[] activeCameras, int nrOfVehicles, VehiclePool vehiclePool, float minDistanceToAdd, float distanceToRemove, float masterVolume, float greenLightTime = -1, float yellowLightTime = -1)
        {
            ...
            //set the nr of jobs based on processor count
            ...
            densityManager = gameObject.AddComponent<DensityManager>().Initialize(trafficVehicles, waypointManager, currentSceneData, activeCameraPositions, nrOfVehicles);
        ...
        public void ClearTrafficOnArea(Vector3 center, float radius)
        {
            ...
            intersectionManager.ResetIntersections(center, radius);
            for (int i = 0; i < vehiclePosition.Length; i++)
            {
                if (vehicleRigidbody[i].gameObject.activeSelf)
                {
                    ...
                    {
                        densityManager.RemoveVehicle(vehicleListIndex[i], true);
                    }
                }
            }
        }
        ...
        {
            densityManager.RemoveVehicle(vehicleListIndex[index], true);
        }
        ...
        private void NewVehicleAdded(int vehicleIndex, Vector3 targetWaypointPosition, float maxSpeed, float powerStep, float brakeStep)
        {
            //set new vehicle parameters
            vehicleTargetWaypointPosition[vehicleIndex] = targetWaypointPosition;
            vehicleMaxSpeed[vehicleIndex] = maxSpeed;
            vehicleSpecialDriveAction[vehicleIndex] = SpecialDriveActionTypes.Forward;
            vehicleActionValue[vehicleIndex] = 0;
            vehiclePowerStep[vehicleIndex] = powerStep;
            vehicleBrakeStep[vehicleIndex] = brakeStep;
            ...
            drivingAI.ResetActions(vehicleIndex);
        }
        ...
        private void DestinationChanged(int vehicleIndex, Vector3 targetWaypointPosition, float maxSpeed, BlinkType blinkType)
        {
            vehicleNeedWaypoint[vehicleIndex] = false;
            vehicleTargetWaypointPosition[vehicleIndex] = targetWaypointPosition;
            vehicleMaxSpeed[vehicleIndex] = maxSpeed;
        ...
        private void FixedUpdate()
        {
            ...
            for (int i = 0; i < nrOfVehicles; i++)
            {
                if (!vehicleRigidbody[i].IsSleeping())
                {
                    ...
                    {
                        vehicleMaxSpeed[i] = drivingAI.GetSpeedMS(i);
                    }
        ...
        private void Update()
        {
            ...
            densityManager.UpdateVehicleDensity();

            if (vehicleReadyToRemove[indexToRemove] == true)
            {
                if (vehicleRigidbody[indexToRemove].gameObject.activeSelf)
                {
                    if (densityManager.RemoveVehicle(vehicleListIndex[indexToRemove], false) == true)
                    {
