...
namespace GleyTrafficSystem
{
    ...
    public class CurrentSceneData : MonoBehaviour
    {
        ...
        /// <returns></returns>
        public int GetNeighborCellWaypoint(int row, int column, int depth, VehicleTypes carType)
        {
            ...
            {
                //get a random cell
                Vector2Int selectedNeighbor = neighbors[UnityEngine.Random.Range(0, neighbors.Count)];
                //get a random waypoint that supports the current vehicle
                List<SpawnWaypoint> possibleWaypoints = grid[selectedNeighbor.x].row[selectedNeighbor.y].spawnWaypoints.Where(cond1 => cond1.allowedVehicles.Contains(carType)).ToList();
                if (possibleWaypoints.Count > 0)
                {
                    return possibleWaypoints[UnityEngine.Random.Range(0, possibleWaypoints.Count)].waypointIndex;
                }
                //else
                //{
                //    Debug.Log("No waypoints found for " + carType);
                //}
            }
            return -1;
        }
