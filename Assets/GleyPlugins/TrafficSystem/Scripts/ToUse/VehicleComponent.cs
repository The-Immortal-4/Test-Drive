...
namespace GleyTrafficSystem
{
    ...
    public class VehicleComponent : MonoBehaviour
    {
        ...
        {
            currentAction = SpecialDriveActionTypes.Forward;
            ...
            //set initial velocity at 1/3 of max velocity
            rb.velocity = transform.forward * maxSpeed / 3 / 3.6f;
            if (engineSound)
            {
        ...
        public void VehicleStoppedReversing()
        {
            if (isInTrigger)
            {
                if (obstacleList.Count > 0)
                {
                    ...
                    VehicleEvents.TriggeerNewObjectInTriggerEvent(isInTrigger, listIndex, carHit, collidingVehicleIndex, false, false, false);
                }
                else
                {
                    isInTrigger = false;
        ...
        /// 
        /// </summary>
        /// <returns>nr of vehicle wheels</returns>
            ...
            if (!other.isTrigger)
            {
                ...
                if (carHit)
                {
                    ...
                    if (other.attachedRigidbody.GetComponent<VehicleComponent>() == null)
                    {
                        ...
                        VehicleEvents.TriggeerNewObjectInTriggerEvent(isInTrigger, listIndex, false, collidingVehicleIndex, false, theySeeEachOther, true);
                        return;
                    }
                    ...
                    VehicleEvents.TriggeerNewObjectInTriggerEvent(isInTrigger, listIndex, carHit, collidingVehicleIndex, false, theySeeEachOther, false);
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            NewColliderHit(other);
                            VehicleEvents.TriggeerNewObjectInTriggerEvent(isInTrigger, listIndex, carHit, collidingVehicleIndex, false, theySeeEachOther, true);
                        }
                        else
                        {
        ...
        {
            if (gameObject.name != "TrafficVehicleBlue")
            {
                if (!other.isTrigger)
                {
                    ...
                    {
                        isInTrigger = false;
                        VehicleEvents.TriggeerNewObjectInTriggerEvent(isInTrigger, listIndex, false, -1, false, false, false);
                    }
                }
            }
        }
        ...
        private void AVehicleChengedState(int vehicleIndex, Collider collider, SpecialDriveActionTypes newAction)
        {
            ...
            if (obstacleList.Contains(collider))
            {
                if (newAction == SpecialDriveActionTypes.Destroyed)
                {
                    ...
                    VehicleEvents.TriggeerNewObjectInTriggerEvent(false, listIndex, false, vehicleIndex, false, false, false);
                }

                if (newAction == SpecialDriveActionTypes.Reverse || newAction == SpecialDriveActionTypes.AvoidReverse)
                {
                    VehicleEvents.TriggeerNewObjectInTriggerEvent(true, listIndex, true, vehicleIndex, true, false, false);
                }

                if (newAction == SpecialDriveActionTypes.StopInDistance || newAction == SpecialDriveActionTypes.StopInPoint)
                {
                    VehicleEvents.TriggeerNewObjectInTriggerEvent(true, listIndex, true, vehicleIndex, false, false, false);
                }
